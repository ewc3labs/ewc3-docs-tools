...
it was true, which is why `DOCS-22` exists — a prose claim about build state is precisely the class
...
- [One template beats three parsers][one-template]: the register shape is DECIDED and every roadmap
  is brought to it, rather than the checker learning each shape it meets. Reverses DOCS-25/DOCS-28.
