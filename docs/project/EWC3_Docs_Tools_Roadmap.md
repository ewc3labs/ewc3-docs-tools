...
| DT-48 | ⬜ planned | A Delivery Index ROW has no ownership gate, so a MIRROR is indistinguishable from a mint | M | — | headings already gate on ownership; rows never did, so every row declares. SX_Coder mirrors `DW-024`/`DW-025` from SX_DW deliberately and documents it twice; HDCTranslators mirrors hub-minted IDs by a convention it settled with PL. Both read as double-mints. **The prefix-level `reference-only` flag does NOT solve this and must not be stretched to** — `series.js:420` drops such a prefix from `declared` precisely so that minting under it still FAILS, and a register may legitimately mirror `DW-024` while accidentally minting `DW-999`. A prefix cannot tell those rows apart, so the signal has to be per-row. Proposed: a `Registry` column naming the owning repo, empty meaning this one — addressable now that columns resolve by header name (`DT-42`). NOT in the State cell, which collapses two dimensions into one scalar, and NOT a marker inside the ID cell, which stops the row regex matching SILENTLY (`DT-27`). Convention is SXC's to define across three registers; the parser is this repo's. Blocked on the convention, not on the code. See `DT-40` |
