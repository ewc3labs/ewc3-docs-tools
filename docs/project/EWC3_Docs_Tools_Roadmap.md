...
| --- | --- | --- | --- | --- |
| DT | global | ewc3-docs-tools | <!--ewc3:lastDT-->DT-47<!--/ewc3:lastDT--> | toolkit features and fixes |
...
| DT-42 | 🟦 coded | Read the Delivery Index columns by HEADER NAME, not by position | S | — | position was hardcoded as `state=1, title=2` against the house style `\| ID \| State \| Slice \|`. HDCTranslators writes `\| ID \| Title \| State \|`, so every document generated for it was named after its state — `HDC-00001_done.md`, all the way down — and nothing errored. Five registers in this estate have five different column layouts |
