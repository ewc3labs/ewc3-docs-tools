...
| DOCS-056 | 🟦 tested | After adoption, editing a Delivery Index row is silently discarded by the next `index --write` | M | [DOCS-056][docs-056] | L1 and L2 built: index --write refuses a hand-edited row, index --check fails a diverged one; 24 controls unit-tested, index --check green locally on all 43 rows here and added to CI; not yet run on an estate register |
