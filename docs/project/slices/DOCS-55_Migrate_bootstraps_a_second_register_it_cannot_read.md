...
est: M
doc: '[DOCS-55](slices/DOCS-55_Migrate_bootstraps_a_second_register_it_cannot_read.md)'
status: 'BLOCKS adopting a migrated register: `migrate` cannot read the `Prefix`-first table `series` reads, so it bootstraps a duplicate that unclaims the owner and re-pads DOCS-54 to DT-00054'
priority: high
lane: migrate
---

# DOCS-55 — `migrate-project` synthesises a second prefix register
...
```
17: ## ID Prefixes     | DT | global | ewc3-docs-tools    | DOCS-54     |   <- authored, correct
...
2. **The padding contradicts the register.** `DOCS-54` becomes `DT-00054`, against `DOCS-44`
   (padding is a convention of a PREFIX, and the shortest id states it) and `DOCS-49` (one spelling
...
duplicate is invisible to the check whose entire job is duplicate declarations — `DOCS-30` one layer
...
question — which is the defect, not the remedy. `DOCS-27` already paid for this lesson: it was fixed
by centralising the id grammar rather than by correcting eighteen regexes.

Related and probably the same slice by the time it is built: `DOCS-32` (one canonical register
template) and `DOCS-42` (the registers disagree on shape).
