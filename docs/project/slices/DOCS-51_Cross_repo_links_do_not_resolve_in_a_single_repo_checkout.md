...
est: M
doc: '[DOCS-51](slices/DOCS-51_Cross_repo_links_do_not_resolve_in_a_single_repo_checkout.md)'
...
lane: links
---
# DOCS-51 — Cross-repo relative links cannot resolve in a single-repo checkout
...
`DOCS-48` already ruled the shape this needs, for sibling *registers*, and it applies unchanged
...
### Do not fetch the twin — it manufactures the class DOCS-48 forbids
...
dead."* That is DOCS-48’s own sentence one layer up. If it is ever done it needs the same three
...
`DOCS-49` required of the canonicality lint before adopting it.
...
what is on disk.** `DOCS-48`’s three outcomes only stay meaningful if the third is not secretly a
...
Until it is decided, this row must not be read as *"worktrees are fixed"*: **DOCS-51 does not make
CI green**, and saying otherwise in the register would be the same misdirection `DOCS-52` is about.
