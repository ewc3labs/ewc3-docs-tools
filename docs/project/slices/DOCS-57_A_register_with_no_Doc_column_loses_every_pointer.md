...
est: M
doc: '[DOCS-57](slices/DOCS-57_A_register_with_no_Doc_column_loses_every_pointer.md)'
...
# DOCS-57 — A register with no `Doc` column loses every pointer to its slices

`DOCS-53` moved the slice pointer out of the Status cell and into `Doc`, on the reasoning that `Doc`
...
a tool that moved the surface out from under it.** Same shape as `DOCS-56`, one level up: not
...
1. **`index` must not silently drop a frontmatter field that has no column.** A document declaring
   `doc:` in a register with no `Doc` column is a mismatch worth naming, not ignoring — `DOCS-27`’s
   refuse-don’t-drop, applied to columns.
2. **Where no `Doc` column exists, the pointer belongs in the last cell**, which is where
   `migrate-project` already puts `See [slice notes](...)`. `DOCS-53` emptied that cell to stop the
...
- `REGISTER_HEADER` **matches** their `| Series | Last Num |` table exactly once, so
  `migrate-project` takes the UPDATE path — `DOCS-55`’s duplicate-register trap cannot fire there.
