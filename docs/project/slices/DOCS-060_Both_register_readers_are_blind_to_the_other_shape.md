...
est: M
doc: '[DOCS-060](slices/DOCS-060_Both_register_readers_are_blind_to_the_other_shape.md)'
...
# DOCS-060 — Both register readers are blind to the other shape

`DOCS-055` recorded that `migrate` cannot read the `Prefix`-first table `series` reads. The inverse
...
That is the measurement `DOCS-048` already recorded without identifying the cause: *"`readSeries`
...
lib/migrate.js   matches  | Series | Last Num |   blind to Prefix-first    DOCS-055
```
...
What is needed is a decision about what the first column MEANS in each template, which is `DOCS-032`
(one canonical register template) and `DOCS-042` (the registers disagree on shape). This slice
...
Sequencing: `DOCS-055`'s fix — split `readSeries` into a text parser and a file wrapper so both
...
- **`lib/migrate.js:188`** — `readSeries.fromText` is not defined anywhere, so the conditional is
  ...
  `DOCS-055`, and **two findings agreeing from different directions is the useful part**.
