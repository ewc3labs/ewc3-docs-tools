...
est: S
doc: '[DOCS-052](slices/DOCS-052_Format_destroys_frontmatter.md)'
...
priority: high
lane: format
blocks: [DOCS-035, DOCS-041]
---

# DOCS-052 — `format` reflows frontmatter as prose, and `check` calls the wreckage correct
...
Authored `docs/project/slices/DOCS-051_*.md` with valid frontmatter, then ran `fix`:
...
after    id: DOCS-051 title: '`links` resolves cross-repo targets from the worktree, not from the
...
**This repo had no frontmatter documents.** The `DOCS-051` slice document above is the first one,
and the specimen in [the-slice-document-is-the-object][the-slice-document] is inside a fenced block,
where `format` correctly leaves it alone. `DOCS-037` (`index`) shipped and was exercised against
...
- **`index` should refuse, not throw.** An unparseable frontmatter block currently produces a raw
  node stack trace naming `lib/frontmatter.js:141`. `DOCS-027` already ruled the shape here — refuse
  loudly and name the file, rather than dropping or crashing.
- **`format --check` should say what would change** — this is `DOCS-002`, and this incident is a
...
C. correct                 ->  PARSED  {"id":"DOCS-051","title":"fine"}
```
...
```
id: DOCS-053 state: ⬜ planned title: A five-line stub est: S doc: —
```
...
1 document(s) claim an id this register never minted:
  DOCS-053 state: ⬜ planned title: A five-line stub est: S doc: —
```
...
**Dependency direction**, by LabsHQ's test — *DOCS-052 blocks anything that AUTHORS frontmatter
documents; it does not block anything that only READS them*: `DOCS-035` and `DOCS-041` author, so
they are blocked. `DOCS-037` (`index`) only reads, so it is **not** blocked and has been removed
from the list.
...
```
docs/project/slices/DOCS-051_....md  ->  [DOCS-051][docs-051]   (undefined reference)
docs/project/slices/DOCS-052_....md  ->  [DOCS-052][docs-052]   (undefined reference)
```
...
now. They are recorded as DOCS-052 evidence rather than worked around, because the alternative was
to write a worse `doc:` value to satisfy a checker that is wrong — **changing correct data to please
a broken instrument**, which is the failure mode this register exists to catch.
...
turned `doc: '[DOCS-001](slices/DOCS-001_....md)'` into `doc: '[DOCS-001][docs-001]'` plus a
definition appended to the **body**. The field still read fine; the definition, now outside the
frontmatter, resolved against the document's own directory, and `links` reported 27 dead targets at
`slices/slices/...`.
...
after    All 53 file(s) formatted correctly · 11 link problems, all DOCS-051 · 155 passing
```
