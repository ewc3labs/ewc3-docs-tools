...
# DOCS-054 — `migrate-project` must never regenerate a slice document a human already authored
...
frozen. `DOCS-051`, `DOCS-052` and `DOCS-053` keep their authored prose, and their register rows
point at the real filenames. 155 passing, 0 failing.
