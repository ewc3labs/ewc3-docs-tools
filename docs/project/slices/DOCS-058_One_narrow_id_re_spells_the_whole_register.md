...
est: M
doc: '[DOCS-058](slices/DOCS-058_One_narrow_id_re_spells_the_whole_register.md)'
...
# DOCS-058 — One narrow id re-spells every other id in the register
...
inspect it. That is `DOCS-053`’s shape a second time: the damage is latent in the emitted documents
and applied by the step documented to follow.

On DevTools this is **92 rows re-spelled** — and by `DOCS-049`’s one-spelling-per-id rule, every
...
`widthOf(prefix)` reads the register and takes the shortest id as the convention — `DOCS-044`’s
...
This is `DOCS-055` with the polarity reversed and both are the same defect: **a width nobody
declared, derived from rows and then written back over them.** `DOCS-055` derived 5 and produced
...
The padding minimum belongs in the Number Series table as a declared pattern — `DOCS-049`’s
...
2. **Take the declared minimum when the register declares one**, and refuse rather than derive when
   it does not — `DOCS-049` already specced the cell.
...
⛔ **DevTools must not adopt until this lands** — `DOCS-057` alone would have cost them their
