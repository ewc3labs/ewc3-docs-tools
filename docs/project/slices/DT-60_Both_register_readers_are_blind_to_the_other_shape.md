...
est: M
doc: '[DT-60](slices/DT-60_Both_register_readers_are_blind_to_the_other_shape.md)'
...
# DT-60 — Both register readers are blind to the other shape

`DT-55` recorded that `migrate` cannot read the `Prefix`-first table `series` reads. The inverse is
also true, and it is worse, because **the code says it was already fixed.**
...
That is the measurement `DT-48` already recorded without identifying the cause: *"`readSeries`
...
lib/migrate.js   matches  | Series | Last Num |   blind to Prefix-first    DT-55
```
...
What is needed is a decision about what the first column MEANS in each template, which is `DT-32`
(one canonical register template) and `DT-42` (the registers disagree on shape). This slice supplies
the measurement those two were missing: it is not a parser preference, it is that **one of the two
templates puts a prefix in column one and the other puts a label there.**

Sequencing: `DT-55`'s fix — split `readSeries` into a text parser and a file wrapper so both
...
- **`lib/migrate.js:188`** — `readSeries.fromText` is not defined anywhere, so the conditional is
  ...
  `DT-55`, and **two findings agreeing from different directions is the useful part**.
