...
# DOCS-055 — `migrate-project` synthesises a second prefix register
...
duplicate is invisible to the check whose entire job is duplicate declarations — `DOCS-030` one
layer up, in the register rather than the index.
...
question — which is the defect, not the remedy. `DOCS-027` already paid for this lesson: it was
fixed by centralising the id grammar rather than by correcting eighteen regexes.

Related and probably the same slice by the time it is built: `DOCS-032` (one canonical register
template) and `DOCS-042` (the registers disagree on shape).
