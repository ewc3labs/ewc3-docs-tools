...
est: M
doc: '[DOCS-56](slices/DOCS-56_After_adoption_editing_the_register_is_silently_futile.md)'
...
# DOCS-56 — After adoption, editing the register is silently futile
...
```
hand-edited DOCS-49 row to carry new text     edit present: true
...
## This is the mirror of `DOCS-53`, and together they are the whole trap

| | authority in practice | what is silently lost |
| --- | --- | --- |
| **before adoption** (`DOCS-53`) | the row, carried into `status:` | the migration — rows come back |
...
until it reached 9,952 characters** — `49b71eb`, `f9d68a1`, `23d2937`, all `DOCS-49`, the last
...
`DOCS-27` already ruled this shape for id cells: *an almost-parsing cell must be REPORTED, never
...
| | owner | |
| --- | --- | --- |
| the silent discard (`DOCS-56`) | **the tool** | authority flips at adoption, nothing announces it |
...
A thin row is **176 bytes** — the number regeneration actually produced for `DOCS-49`. Losing 176
...
> **`migrate-project` does NOT recover a diverged row, by design.** `DOCS-54` stops it regenerating
...
PMO's branch still holds `DOCS-49` at 10,168 characters, and it is **deliberately frozen there**,
...
defect that `DOCS-30`, `DOCS-48` and this slice are all circling — and doing it while believing they
are cleaning up.

## Correction — the claim was "byte for byte" and it decayed

PMO fetched `497a9ea`, read the DOCS-49 body against their own `23d2937` register cell, and found
...
measured at `dd6273d`, before `DOCS-52`'s fix existed. `497a9ea` then ran `format` across all 40
