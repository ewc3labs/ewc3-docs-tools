...
est: S
doc: '[DOCS-53](slices/DOCS-53_Migration_output_fed_to_index_restores_every_row.md)'
...
# DOCS-53 — `migrate-project` output fed to `index` restores every row it just moved out
...
```
| DOCS-49 | ⬜ planned | Read the canonical Number Series table... | M | — | See [slice notes](slices/DOCS-49_....md). |
```
...
**Sequencing:** this blocks bulk migration of this repo’s own four monster rows (`DOCS-49`,
`DOCS-48`, `DOCS-27`, `DOCS-50` — 17,101 characters between them) and any other repo, because the
migration cannot currently be followed by the command that regenerates registers.

Related: `DOCS-26` (`migrate-project` emits from ONE source) and `DOCS-34` (`migrate-project`
accepts shapes `series` refuses) are both about this command being trusted further than it has been
tested.
...
frontmatter as prose and reports a link in a field value as an undefined reference — `DOCS-52`. It
is greppable today and becomes a link when that lands. An authored `Doc` value is never replaced:
`DOCS-5` keeps `[Adopting](../Adopting.md)`.
...
Until then the four monster rows in this register — `DOCS-49`, `DOCS-48`, `DOCS-27`, `DOCS-50`,
17,101 characters between them — can be migrated whenever wanted; the summaries are the human half.
