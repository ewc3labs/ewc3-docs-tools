...
**`DOCS-30` becomes CHEAP, not automatic** — and the first version of this paragraph overclaimed.
...
being structural. **So duplicate-ID validation is a REQUIRED part of `DOCS-35`, not a consequence of
...
### Which answers `DOCS-15`, parked since it was minted

`DOCS-15` (`next <PREFIX>`) sits at *"not yet earned — derived Last Used already removed the state
...
### ⚠️ Sequencing: this model WIDENS `DOCS-30` before it closes it

`DOCS-30` — a same-repo duplicate declaration is undetectable — is **still live, and the two-surface
...
`id:` without one file overwriting the other. **So `DOCS-35` should land before anything that adds a
...
1. ✅ **SETTLED — the slice document is the authority.** *(Wilson, 2026-09-02; see [One thing to
   ...
   **Codex was right that deferring this made the rule unimplementable, not merely undecided.** When
   ...
   and false of the *checker* — a distinction I collapsed. It is a prerequisite for `DOCS-35`, and
...
4. ⚠️ **TWO DOCUMENTS IN THIS BRANCH DISAGREE ABOUT WHETHER THE EMITTED SLICES ARE COMMITTED**, and
   ...
   tree — which is its own argument for deriving claims about build state (`DOCS-22`).
