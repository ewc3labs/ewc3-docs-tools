...
4. **Cross-repo trailer scan** overlaps **DOCS-16** (org-level series check). Same traversal, two
   uses.
5. **⚠️ Cross-repo pinning breaks the `FIX` canon** *(found by SD, 2026-08-24)*. The registry keeps
   ...
   **`(SXdb, Record)`** — an identifier unique only within its scope, qualified by that scope when
   ...
   is the worst option.
6. **`next <PREFIX>` was deferred as DOCS-15** (*"only the +1 is manual"*).
...
every other derived fact. **And C's stated cost — the N-repo scan — is DOCS-16**, already on the
Delivery Index. §9 Q4 spots the overlap and undersells it: DOCS-16 stops being speculative and
...
object, a declaration is a file"* while [DOCS-35][frontmatter-is] was landing the stronger form — a
...
**DOCS-30 is still live, and this model makes it likelier before it makes it impossible.** Measured
...
surfaces now exist by design, so the window is wider than when DOCS-30 was written. Under
frontmatter-as-declaration it closes structurally — two documents cannot carry the same `id:`
without one file overwriting the other — which is an argument for sequencing DOCS-35 ahead of
...
That also gives [`DOCS-15`][dt-15-note] its answer. It is currently parked as *"not yet earned —
