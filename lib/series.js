...
function withoutFences(text) {
	...
	return text.split('\n').map((line) => {
		...
		if (s.startsWith('```') || s.startsWith('~~~')) { fence = s.slice(0, 3); return ''; }
		return line;
	}).join('\n');
}
...
function frozenViolations(root, specs) {
	...
	for (const r of read) {
		for (const [prefix, n] of r.used) {
			if (!usedAnywhere.has(prefix) || n > usedAnywhere.get(prefix)) { usedAnywhere.set(prefix, n); }
		}
	}
	...
	for (const r of read) {
		for (const [prefix, scope] of r.scopes) {
			...
			const highest = usedAnywhere.get(prefix);
			if (highest !== undefined && highest > scope.ceiling) {
				out.push({ file: r.file, prefix, ceiling: scope.ceiling, highest });
			}
		}
	}
	return out;
