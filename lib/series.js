...
function withoutFences(text) {
	...
	return text.split('\n').map((line) => {
		...
		if (fence === null && s.startsWith('<!--') && !s.includes('-->')) { inComment = true; return ''; }
		if (fence === null && s.startsWith('<!--') && s.includes('-->')) { return ''; }
...
function headingDeclarations(text, declared) {
	const out = [];
	if (!declared || !declared.size) { return out; }
	const rx = /^#{2,4}\s+\*{0,2}`?([A-Z][A-Z0-9]{0,7})-(\d+)[A-Za-z]?`?/gm;
	let m;
	while ((m = rx.exec(text)) !== null) {
		if (declared.has(m[1])) { out.push({ prefix: m[1], num: Number.parseInt(m[2], 10), index: m.index }); }
	}
	return out;
}
