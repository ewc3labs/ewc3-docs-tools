...
function withoutFences(text) {
	// Track the opening RUN, not just three characters. Found by codex: a four-backtick block
	// containing a documented three-backtick example was closed by the inner fence, leaking whatever
	// followed - so a document explaining fenced examples could still mint an ID from one.
	let fence = null;
	return text.split('\n').map((line) => {
		const s = line.trim();
