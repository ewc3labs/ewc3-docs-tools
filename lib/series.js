 ...
 */
const OWNERSHIP_ROW = /^\|\s*`?([A-Z][A-Z0-9]{0,7})(?:-N+)?`?\s*\|\s*([^|]+?)\s*\|/gm;
...
function readSeries(file) {
	...
	if (headerAt !== -1) {
		...
		// A third register shape, and the most common one in this estate: column 1 is a human NAME
		// ("Vertical Slices", "Hotfixes", "DevTools") and the prefix appears only inside the
		// `Last Num` cell - `VS-421`, `DT-065`, and sometimes two at once, `TS-02 · DW-025`.
		//
		// Read THAT COLUMN specifically, never the whole row. The Series Description beside it is
		// prose about other repositories' prefixes - SX_Coder's cross-project row names `AIR`, `DW`
		// and `DT` while owning none of them - so a row-wide scan would have the hub declare exactly
		// the series it has just finished handing away. A mention is not a mint, in a register too.
		const tableLines = table.split('\n');
		const headerCells = rowCells(tableLines[0] || '');
		const lastNumAt = headerCells.findIndex((c) => /last\s*num/i.test(c));
		const scopeAt = headerCells.findIndex((c) => /^\s*\**\s*scope\s*\**\s*$/i.test(c));
		if (lastNumAt !== -1) {
			if (declared.size === 0) { legacyShape = true; }
			for (const line of tableLines.slice(1)) {
				const cell = rowCells(line)[lastNumAt];
				if (!cell || /^-+$/.test(cell.trim())) { continue; }
				for (const hit of cell.matchAll(/\b([A-Z][A-Z0-9]{0,7})-\d+\b/g)) {
					if (!HEADER_WORDS.has(hit[1].toUpperCase())) { declared.add(hit[1]); }
				}
		}
		}
		...
		if (scopeAt !== -1) {
			for (const line of tableLines.slice(1)) {
				...
				const numCell = lastNumAt === -1 ? '' : (cells[lastNumAt] || '');
				if (!scopeCell || /^-+$/.test(scopeCell)) { continue; }

				// Which series is this row about? The first cell as a bare prefix or mint template,
				// else the prefix named in its own Last Num cell.
				const first = (cells[0] || '').trim();
				const byFirst = /^`?([A-Z][A-Z0-9]{0,7})(?:-N+)?`?$/.exec(first);
				const byNum = /\b([A-Z][A-Z0-9]{0,7})-\d+\b/.exec(numCell);
				const prefix = byFirst ? byFirst[1] : (byNum ? byNum[1] : null);
				...
				const ceilingMatch = /(\d+)/.exec(frozen ? (scopeCell + ' ' + numCell) : numCell);
				scopes.set(prefix, {
					declared: scopeCell,
					local,
					frozen,
	...
	return { file, declared, used, legacyShape, scopes };
}
