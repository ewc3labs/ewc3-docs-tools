...
const RESOLVERS = {
	 ...
	 */
	/**
	 * `pad` is the series' DECLARED minimum width, supplied by `resolveValues` from
	 * `series.widths` - never inferred from the ids themselves.
	 *
	 * Deriving it is circular: the width would come from the population it is about to render, so one
	 * narrow id silently restates the convention for the whole series. That is `DOCS-058`, where a
	 * deliberate two-digit row dragged 92 three-digit ids down to two.
	 *
	 * Declared, the marker renders at the same width as the ids it counts. Derived, it rendered the
	 * marker UNPADDED beside padded ids - two spellings of one number, which is the defect
	 * `DOCS-049` exists to prevent, and a grep for either misses the other.
	 *
	 * (This comment originally illustrated that with a literal narrow id. The padding sweep rewrote
	 * it, because a padder cannot tell an id from a quotation of one - `DOCS-055` and `DOCS-058` are
	 * about exactly that, and the rule is stated in prose here so nothing can eat it.)
	 */
	lastId({ prefix, files, pad = 0 }, root) {
		if (!prefix) { throw new Error('lastId: a "prefix" is required, e.g. {"prefix": "PQ"}'); }
		return String(lastNumber(root, prefix, files)).padStart(pad, '0');
	},
...
function resolveValues(config, root) {
	...
	// ONE DECLARATION OF THE WIDTH, READ WHEREVER A NUMBER BECOMES AN ID.
	//
	// `series.widths` is the single place a prefix's minimum width is written down. Copying it into
	// each `lastId` spec would be a hand-maintained duplicate of a fact the config already holds -
	// the class this toolkit exists to remove - and the two copies would disagree the first time
	// one changed. `migrate-project` reads the same map for filenames.
	const widths = (config.series || {}).widths || {};
	...
	const resolveOne = ([name, spec]) => {
		...
		let args = spec[kind];
		if (kind === 'lastId' && args && args.pad === undefined && widths[args.prefix]) {
			args = { ...args, pad: widths[args.prefix] };
		}
		out[name] = RESOLVERS[kind](args, root, out);
	};
...
/**
 * Character ranges that are CODE: fenced blocks and inline spans.
 *
 * A marker written inside code is documentation ABOUT the syntax, not the syntax. Without this, the
 * one repository most likely to explain how markers work - this one - cannot pass its own check,
 * and any project documenting the convention has to choose between correct docs and green CI.
 */
function codeRanges(text) {
	const ranges = [];
	const lines = text.split('\n');
	// Which lines are fenced is decided once, with look-ahead, in lib/fence.js. Keeping only the first
	// three characters of an opener let a ``` example inside a ```` block close it, and the marker after
	// it was substituted; reading an unclosed indented `    ~~~` as a fence would erase the rest.
	const fenced = fencedLineSet(lines);

	let offset = 0, runStart = null;
	lines.forEach((line, i) => {
		if (fenced.has(i)) {
			if (runStart === null) { runStart = offset; }
		} else {
			if (runStart !== null) { ranges.push([runStart, offset - 1]); runStart = null; }
	...
	if (runStart !== null) { ranges.push([runStart, text.length]); }
	return ranges;
}
