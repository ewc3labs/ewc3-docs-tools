...
function codeRanges(text) {
	const ranges = [];
	let offset = 0, fenceStart = null, fence = null;

	for (const line of text.split('\n')) {
		// The fence grammar lives in lib/fence.js. Keeping only the first three characters of the opener
		// let a ``` example inside a ```` block close it, and the example marker after it was substituted.
		const opener = fence === null ? fenceOpener(line) : null;
		if (fence !== null) {
			if (closesFence(line, fence)) { ranges.push([fenceStart, offset + line.length]); fence = null; }
		} else if (opener) {
			fence = opener;
			fenceStart = offset;
		} else {
