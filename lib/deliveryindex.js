 ...
 * repository's. A definition pointing anywhere else resolves to a different link.
 *
 * ONLY THE SIMPLE SUBSET IS RESOLVED, on purpose. Four review rounds on PR #6 each found another corner
 * of CommonMark a hand-rolled resolver got wrong (HTML blocks, fences inside them, escaped brackets,
 * angle destinations, nested backtick runs), and each wrong corner made two different renders compare
 * equal. So resolution covers what `format` actually emits - a bare destination with no title, in a
 * cell with no code - and everything else is compared exactly as written. That direction can only make
 * an equal cell read as changed, which fails loudly; it can never pass a real change.
 */
...
/**
 * Reference definitions in a document: label -> bare target, or null for a definition outside the simple
 * subset (an angle-bracketed destination, one with whitespace, parentheses or a backslash, or a title).
 * A null still CLAIMS its label: first one wins, as in CommonMark, so a later simple definition must not
 * take over a label GitHub resolves to the earlier one.
 *
 * A definition inside a fence defines nothing, and an AMBIGUOUS fence counts as a fence: for a gate,
 * trusting a doubtful definition could make a changed cell look equal, while distrusting it can only
 * make an equal cell look changed - which is loud.
 */
function referenceDefs(text) {
	...
	lines.forEach((line, i) => {
		...
		if (!m) { return; }
		// `[^1]: ...` is a GitHub footnote, not a link definition; it claims no link label.
		if (m[1].startsWith('^')) { return; }
		const key = linkLabel(m[1]);
		const simple = /^[^\s<>()\\]+$/.test(m[2]) && !m[3].trim();
		if (!defs.has(key)) { defs.set(key, simple ? m[2] : null); }
	});
	return defs;
}
...
/**
 * `[text][label]` rewritten as the `[text](target)` it renders as - and NOTHING ELSE.
 *
 * The subset is defined by INCLUSION. Defined by exclusion, five review rounds on PR #6 each found a
 * form it had not excluded: HTML blocks, fences inside them, escaped brackets, angle destinations,
 * nested backtick runs, footnotes, HTML attributes. The only form `format` writes is a full reference
 * with plain text and a plain label, so that is the only form resolved:
 *
 *   - the cell holds no backtick, `<` or backslash (code, HTML and escapes are never in play);
 *   - text and label hold no bracket, and neither starts with `^` (a footnote, not a link);
 *   - the reference touches no other bracket, `!` or `(` (nesting, images, a following destination).
 *
 * Shortcut `[x]` and collapsed `[x][]` references are not resolved. Anything outside the subset is
 * compared as written, which can only make an equal cell read as changed.
 */
function resolveReferences(cell, defs) {
	if (!defs || !defs.size || !cell.includes('][') || /[`<\\]/.test(cell)) { return cell; }
	return cell.replace(/(?<![\[\]!])\[([^\[\]^][^\[\]]*)\]\[([^\[\]^][^\[\]]*)\](?![\[\]\(])/g, (whole, text, ref) => {
		const def = defs.get(linkLabel(ref));
		return def ? `[${text}](${def})` : whole;
	});
}
...
function renderIndex(text, records) {
	...
	return {
		...
		idColumn: map.id === undefined ? 0 : map.id,
		ok: true,
	};
}
