...
const {
	section, rowCells, headerMap, columnKey, escapeCell, indexHeadings, idPattern, pointerCell, sliceHref,
} = require('./slices.js');
...
function detectWidths(text) {
	// An example row is documentation: it must not set a series' width either (Codex P1, PR #28).
...
function renderRow(headings, map, data, widths) {
	// A register with no Doc column points at the document from its LAST cell, when the document leaves
	// that cell empty - the same string migrate writes (DOCS-065). `__file` is the document's filename.
	const noDocColumn = !headings.some((h) => columnKey(h) === 'doc');
	...
	const cells = headings.map((h, i) => {
		...
		if (empty && noDocColumn && i === last && data.__file) { return pointerCell(data.__file); }
		// An empty Doc cell links to the document too. A kept document with valid frontmatter but no `doc:`
		// otherwise rendered with nothing pointing at it, and was an orphan after adoption (Codex P1, PR #8).
		if (empty && columnKey(h) === 'doc' && data.__file) { return `[${data.id}](${sliceHref(data.__file)})`; }
		return empty ? '' : escapeCell(String(v));
	});
 ...
 *
 * NO LINK SYNTAX IS PARSED. `format` rewrites `index`'s inline `[DOCS-001](slices/...)` as
 * `[DOCS-001][docs-001]` plus a definition, so a formatted register differs from its render in every
 * Doc cell. The first answer resolved references back to inline links, and six review rounds on PR #6
 * found six CommonMark corners where that made two different renders compare equal. The gate instead
 * accepts a row equal to the render OR to what `format` makes of the render (see the CLI) - two exact
 * comparisons, no resolver, and nothing a real change can hide behind.
 */
function gfmCells(line, columns = 0) {
	const cells = rowCells(line).map((c) => c.replace(/^[ \t]+|[ \t]+$/g, '').split('\\|').join('|'));
 ...
 *
 * A REGION THAT DOES NOT RENDER IS NOT A ROW. A fenced example of a row, or one commented out, was read as a
 * declaration: the duplicate check then reported an example colliding with the row it documents, and migration
 * refused a valid roadmap over it (Codex P1, PR #28). Detection runs on `withoutFences`, which keeps the line
 * count, so `line` and `raw` still refer to the real document.
 */
function indexRows(text) {
	const source = text.split('\r\n').join('\n');
	const lines = source.split('\n');
	const visible = withoutFences(source).split('\n');
	const sec = section(lines, 'Delivery Index');
	const out = [];
	if (!sec) { return out; }
	for (let i = sec.start; i < sec.end; i++) {
...
function renderIndex(text, records) {
	...
	// Detection runs on what RENDERS; the rewrite still targets the real lines, which `withoutFences` keeps in step.
	// A fenced or commented example row inside the section read as a row - and this replaces every row it finds
	// between the first and the last, so an example could have been rewritten (Codex P1, PR #28).
	const shown = withoutFences(lines.join('\n')).split('\n');
	const sec = section(lines, 'Delivery Index');
	if (!sec) {
...
module.exports = {
	renderIndex, renderRow, detectWidth, detectWidths, ordering, gfmCells, indexRows,
};
