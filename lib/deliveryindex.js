...
function resolveReferences(cell, defs) {
	...
	// A cell with any backtick is not resolved at all. Code-span boundaries need matching run lengths,
	// and getting them wrong resolved a reference inside code (Codex, PR #6); outside the simple subset.
	if (cell.includes('`')) { return cell; }
	// The lookbehind admits an opening bracket only after an EVEN run of backslashes: `\[` is literal
	// text on GitHub, `\\[` is a literal backslash and a live bracket (Codex, PR #6).
	return cell.replace(
		/(?<=(?:^|[^\\])(?:\\\\)*)\[((?:[^\][\\]|\\.)*)\](?:\[((?:[^\][\\]|\\.)*)\])?(?![(:])/g,
		(whole, text, ref) => {
