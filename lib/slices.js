		...
		for (const m of g.members) {
			...
			if (!isAnchor) {
				...
				if (ev.kept.length || pull.kept.length || ev.apart.length || pull.apart.length) {
					...
					if (apart.length) {
						const named = [...new Set(apart.flatMap((e) => e.ids))].sort()
							.map((k) => `${written.get(k) || k} was minted (${dating.mint.get(k)})`);
						out.push(`### Mentioned before ${named.join(', and ')}`);
						out.push('');
						out.push('_Most likely an earlier, unminted use of the number for other work. Kept for the record;');
						out.push('**not** evidence for this slice._');
						out.push('');
						files('apart');
					}
				}
