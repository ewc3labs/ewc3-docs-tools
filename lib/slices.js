...
function extractSlices(text, { statusText = '', pullupText = '', width = 5, sourceName = '',
	...
	for (const g of groups) {
		...
		for (const m of g.members) {
			...
			const held = existing.get(idText);
			const name = held || `${idText}_${slug(own) || 'slice'}.md`;
			...
			// the body, so it belongs in `Doc` - the column that means exactly this. Written as a
			// PLAIN PATH rather than a markdown link, because `links` reads frontmatter as prose and
			// reports a link in a field value as an undefined reference (DT-52); a path is greppable
			// today and can become a link once that lands. An authored Doc value is never replaced.
			const docKey = columnKey('Doc');
			if (!data[docKey] || /^[—–-]$/.test(String(data[docKey]).trim())) {
				data[docKey] = `slices/${name}`;
			}
			...
			if (!isAnchor) {
				...
				const lastCell = m.row ? m.row.cells.length - 1 : -1;
				const status = m.row ? unescapeCell(m.row.cells[lastCell] || '') : '';
				if (status && status !== (m.row && m.row.title)) {
					out.push(status);
					out.push('');

					// THE NARRATIVE NOW LIVES IN THE BODY, SO IT MUST NOT ALSO STAY IN FRONTMATTER.
					//
					// Frontmatter is the declaration, so `index` renders the Status cell from it. Carrying
					// the original cell across verbatim AND writing the same text into the body meant
					// `migrate-project` emitted a one-line register whose documents declared the paragraph
					// it had just moved out - and the documented next step, `index --write`, put every one
					// of them straight back. Measured on this repo: longest row 341 chars after migrating,
					// 5961 after re-indexing. The migration was undone by the command that follows it.
					//
					// Emptied rather than deleted: the field is what a human - or a summarizer - still owes,
					// and an empty Status cell is visibly unfinished, which is the honest state for a
					// migration awaiting judgement. Only the cell that actually reached the body is cleared,
					// so a family member whose row was never rendered into a narrative keeps its own.
					const key = columnKey(headings[lastCell] || '');
					if (key && data[key] === status) { data[key] = ''; }
				}
			...
			if (held) { kept.push(idText); continue; }
	...
	for (const g of groups) {
		for (const m of g.members) {
			...
			cells[cells.length - 1] = ` See [slice notes](slices/${target}). `;
	...
	return { text: lines.join('\n'), docs, kept, rowCount: rows.length, noteCount: notes.length };
}
