...
function pointerCell(file) {
	return `See [slice notes](slices/${file}).`;
}
 ...
 */
function rebaseRelative(markdown) {
	...
	return lines.map((line, i) => {
		...
		return line.split(/(`+[^`]*`+)/).map((part, k) => (k % 2 ? part
			: part.replace(/(!?\[[^\]]*\]\()(<[^>]*>|[^)\s]+)/g, (w, pre, target) => pre + rebase(target)))).join('');
	}).join('\n');
}
...
function definitionsFor(content, defs, deeper) {
	...
	for (const m of content.matchAll(/\]\[([^\]]+)\]/g)) {
		...
		if (deeper && isRelativeTarget(bare)) { target = target.startsWith('<') ? `<../${bare}>` : `../${bare}`; }
		out.push(`[${m[1]}]: ${target}`);
	}
	return out;
}
			...
			if (!isAnchor) {
				...
				if (ev.length || pull.length) {
					...
					if (ev.length) {
						...
						out.push('');
						ev.forEach((l) => out.push(`- ${quoteEvidence(l)}`));
						out.push('');
					}
					if (pull.length) {
						out.push('**`config/STATUS-pullup.yaml`** (child repos)');
						out.push('');
						pull.forEach((l) => out.push(`- ${quoteEvidence(l)}`));
						out.push('');
					}
				}
			}
...
module.exports = {
	...
	ID_SUFFIX, ID_PREFIX, idPattern, normalizeId, inventorySlices, pointerCell, rebaseRelative, quoteEvidence,
};
