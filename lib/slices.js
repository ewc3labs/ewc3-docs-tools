...
function inventorySlices(entries) {
	...
	return entries.map(({ name, text }) => {
		...
		try {
			...
			return { name, frontmatter: 'unreadable', reason: err.message, id: fromName(name), idFrom: 'filename' };
		}
		if (!parsed.had) { return { name, frontmatter: 'none', id: fromName(name), idFrom: 'filename' }; }
		const declared = parsed.data && parsed.data.id !== undefined ? normalizeId(parsed.data.id) : null;
		if (!declared) { return { name, frontmatter: 'no-id', id: fromName(name), idFrom: 'filename' }; }
...
function extractSlices(text, { statusText = '', pullupText = '', width = 5, sourceName = '',
	existing = new Map(), legacy = new Map() } = {}) {
...
module.exports = {
	...
	ID_SUFFIX, ID_PREFIX, idPattern, normalizeId, inventorySlices,
};
