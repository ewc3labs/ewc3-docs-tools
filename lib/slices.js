 ...
 */
/**
 * Is this id mentioned anywhere in `text` on or after `mintDate`, counting EVERY occurrence? DOCS-078.
 *
 * The backfill hint is a claim about the whole file, so it cannot be read off the rendered evidence: identical text
 * recorded twice, once before the row and once after, is deduplicated to its first occurrence, and the later one
 * would never be seen (Codex, PR #24). An undated line cannot be shown to predate anything, so it counts as after.
 */
function mentionedFrom(text, idKey, dates, mintDate) {
	...
	return text.split('\n').some((raw, i) => {
		...
		let hit = false;
		while ((m = ID.exec(raw)) !== null) {
			if (`${m[1]}-${Number.parseInt(m[2], 10)}${m[3] || ''}` === idKey) { hit = true; break; }
		}
		if (!hit) { return false; }
			...
			if (!isAnchor) {
				...
				if (ev.kept.length || pull.kept.length || ev.apart.length || pull.apart.length) {
					...
					if (apart.length) {
						...
						const one = involved.length === 1;
						out.push(`### Recorded before ${one ? 'this row' : 'these rows'} existed (${[...new Set(involved.map((k) => dating.mint.get(k)))].join(', ')})`);
						out.push('');
						out.push(`_These lines name ${named.join(' and ')} but were recorded before ${one ? 'that row' : 'those rows'} first appeared._`);
						...
						out.push('');
						// The one thing the DATA does say: no mention at all from the row's date on is what a backfill
						// looks like. PER ID - one id's shape is not hidden by another id's later line (Copilot, PR #24).
						const backfilled = involved.filter((k) => !mentionedFrom(statusText, k, dating.status, dating.mint.get(k))
							&& !mentionedFrom(pullupText, k, dating.pullup, dating.mint.get(k)));
						if (backfilled.length) {
							const list = backfilled.map((k) => `${written.get(k) || k} (${dating.mint.get(k)})`).join(' and ');
							out.push(`_Every line for ${list} predates its row, which is what ${backfilled.length === 1 ? 'a row' : 'rows'} written`);
							out.push('down after the work looks like._');
							out.push('');
						}
						files('apart');
					}
