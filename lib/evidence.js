...
function mintDates(repo) {
	...
	const log = run(repo, ['-c', 'core.quotepath=off', 'log', '--reverse', '--no-color', '--no-ext-diff', '--no-renames',
		'--format=%x1e%as %H', '-p', '--unified=0', '--', 'docs/project', ':(exclude)docs/project/slices']);
	if (!log.ok) { return { ok: false, reason: 'git log failed', dates: new Map() }; }
	const dates = new Map();
	const mintedBy = new Map();
	let first = null;
	for (const commit of log.out.split('\x1e')) {
		// `%as %H`, with an object id of whatever length this repository uses - SHA-1 or SHA-256 (Codex, PR #23).
		const head = /^(\d{4}-\d{2}-\d{2}) ([0-9a-f]+)/.exec(commit);
		if (!head) { continue; }
		const [, date, sha] = head;
		if (!first) { first = { date, sha }; }
		for (const line of commit.split('\n')) {
			...
			if (id && !dates.has(id)) { dates.set(id, date); mintedBy.set(id, sha); }
		}
	}
	// A TREE WHOSE REGISTER ALL ARRIVED AT ONCE CANNOT DATE ANYTHING (DOCS-077). An exported tree re-initialised
	// (`git archive`, then `git init`) is not shallow, so every id reads as minted on the day of the import: lines
	// carrying an older date were ALL set apart, and undated ones never were. The dates are an artifact of the import,
	// so they are refused rather than used. A repository genuinely created in one commit reads the same way, and
	// correctly: its ids were not minted at different times, so no line can be shown to predate one.
	//
	// The test is which COMMIT added each row, not its calendar date: a second commit on the same day is still a
	// second mint, and dating must stay on for it (Codex, PR #23).
	const rootCommits = run(repo, ['rev-list', '--max-parents=0', 'HEAD']).out.split('\n').map((s) => s.trim());
	if (mintedBy.size && first && rootCommits.includes(first.sha) && [...mintedBy.values()].every((s) => s === first.sha)) {
		return {
			ok: false, dates: new Map(),
			reason: `this tree's history begins with its register - every id's row arrives in the first commit (${first.date})`,
		};
	}
...
function lineDates(repo, relFile, text) {
	...
	if (blame.ok) {
		...
		for (const l of blame.out.split('\n')) {
			// SHA-1 or SHA-256 object ids (Codex, PR #21).
			const head = /^(?:[0-9a-f]{64}|[0-9a-f]{40}) \d+ (\d+)/.exec(l);
