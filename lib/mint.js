...
function lastUsedCell(text, prefix) {
	...
	// `Last Used` and `Last Num` are the same column under two names. Reading only the first turned an accepted
	// register into a SILENT collision: the counter said AIR-28, the highest retained row said AIR-27, and the
	// mint handed back 28 (Codex P1, PR #30). `Next` is deliberately not read - it holds the id NOT yet used.
	const at = rowCells(lines[header]).findIndex((c) => /last\s*(used|num)/i.test(c));
	if (at === -1) { return 0; }
