...
const { roadmapFiles, readSeries, withoutFences } = require('./series.js');
...
/** The number in a register's `Last Used` cell for `prefix`, or 0. It can run ahead of the rows. */
function lastUsedCell(text, prefix) {
	const lines = withoutFences(text).split('\n');
	const header = lines.findIndex((l) => /^\|\s*(Prefix|Series)\s*\|/i.test(l.trim()));
	if (header === -1) { return 0; }
	const at = rowCells(lines[header]).findIndex((c) => /last\s*used/i.test(c));
	if (at === -1) { return 0; }
...
function planMint({ repo, config = {}, prefix, title, state, sets = [], table }) {
	...
	const scope = series.map((s) => s.scopes.get(prefix)).find(Boolean);
	if (scope && scope.frozen) {
		return refuse(1, `${prefix} is FROZEN (${scope.declared}) - its register retired the series; mint new work elsewhere`);
	}
	if (!declared) {
	...
	const named = (h) => allTables.filter((x) => x.table.heading.toLowerCase() === String(h).trim().toLowerCase());
	const names = allTables.map((x) => `"${x.table.heading || '(no heading)'}"`).join(', ');
	if (table !== undefined) {
	...
	// COLUMNS. --set names a real column of that table; id, state, title and doc are derived or have flags.
	...
	for (const [k] of pairs) {
		...
		if (!keys.includes(k)) { return refuse(1, `--set ${k}: "${target.table.heading}" has no such column; it has ${headings.join(', ')}`); }
	}
	...
	if (!st) {
		...
		st = states.sort((a, b) => states.filter((x) => x === b).length - states.filter((x) => x === a).length)[0] || 'planned';
	}
	...
	return {
		...
		roadmapText: crlf ? rendered.text.split('\n').join('\r\n') : rendered.text,
	};
}
