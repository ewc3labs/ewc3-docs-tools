...
const { section, rowCells, headerMap, indexHeadings, normalizeId } = require('./slices.js');
...
/**
 * A register's legend, word -> spelling, from its `## State Legend` bullets: `- <glyph> \`<word>\` — ...`.
 * A bullet whose text starts with `_` is a note (a reserved word), not a state. Empty when there is no legend.
 */
function legendOf(text) {
	const lines = String(text).split('\r\n').join('\n').split('\n');
	const sec = section(lines, 'State Legend');
	const out = new Map();
	if (!sec) { return out; }
	for (let i = sec.start + 1; i < sec.end; i++) {
		const m = /^\s*[-*+]\s+(.*?)`([A-Za-z][A-Za-z0-9-]*)`/.exec(lines[i]);
		if (!m || m[1].trim().startsWith('_')) { continue; }
		const glyph = m[1].trim();
		const word = m[2].toLowerCase();
		if (!out.has(word)) { out.set(word, glyph ? `${glyph} ${word}` : word); }
	}
	return out;
}
...
function planFold({ repo, sliceDir, roadmaps, sinceShas = null }) {
	...
	for (const t of texts) { for (const [w, s] of legendOf(t)) { if (!legend.has(w)) { legend.set(w, s); } } }
	...
	for (const { sha, lines } of gitEvents(repo)) {
		...
		for (const p of pairs) {
			...
			const resolved = resolveState(p.state, legend, spellings);
	...
	for (const [id, ev] of newest) {
		...
		if (doc.data.state_source === 'human') {
			if (current !== ev.value) { conflicts.push({ id, file: doc.file, human: current, trailer: ev.value, sha: ev.sha }); }
			continue;
		}
		const entry = { id, file: doc.file, text: doc.text, data: doc.data, from: current, to: ev.value, sha: ev.sha };
...
module.exports = { parseTrailers, legendOf, wordOf, resolveState, planFold, applyFold };
