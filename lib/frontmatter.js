...
/**
 * Would this value survive being written plain and read back?
 *
 * The round-trip IS the test, so the rule cannot drift from the parser. Found by codex: writing
 * `title: "Fix the #4 lookup"` back unquoted yields `title: Fix the #4 lookup`, which re-reads as
 * "Fix the" - so a fold updating only `state` would silently truncate an unrelated title.
 */
function survivesPlain(v) {
	return scalar(v) === v;
}
...
function stringify(data) {
	...
	for (const [k, v] of (data instanceof Map ? data : Object.entries(data))) {
		...
		if (Array.isArray(v)) {
			const items = v.map((x) => (survivesPlain(String(x)) ? x : JSON.stringify(String(x))));
			lines.push(items.length ? `${k}: [${items.join(', ')}]` : `${k}: []`);
			continue;
		}
		const s = String(v);
		lines.push(`${k}: ${survivesPlain(s) ? s : JSON.stringify(s)}`);
	}
	return lines.join('\n');
}
