...
function read(text) {
	...
	// Consume EXACTLY the newline that terminates the closing-fence line, and nothing more. An
	// earlier version stripped every leading blank line and `write` added back one, so a document
	// with two blank lines after its frontmatter lost one on a state-only fold - breaking the
	// byte-for-byte body contract this function's own docstring claims.
	const after = norm.slice(end + 1 + FENCE.length).replace(/^\n/, '');
	return { data: parse(block), body: after, had: true, block };
}
...
function patch(block, data) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		// Gather this field: its own line plus any block-list items beneath it.
		const start = i;
