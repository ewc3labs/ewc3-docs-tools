...
function normalize(block) {
	...
	const out = block.split('\n').map((line) => {
		...
		if (!item && value.startsWith('[') && value.endsWith(']')) {
			...
			// Only the unsafe elements are quoted; a quoted or safe element is kept exactly as written, so a
			// repair does not churn its neighbours (Copilot, PR #10).
			return `${head}[${parts.map((p) => (unquote(p) !== null || yamlFlow(p) ? p : quoted(p))).join(', ')}]${comment}${cr}`;
		}
