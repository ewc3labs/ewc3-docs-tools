...
const fs = require('fs');
const { fenceOpener, closesFence } = require('./fence');
...
function markedLines(lines) {
	...
	lines.forEach((line, i) => {
		...
		// The opening RUN is kept, not its first three characters, so a ``` example inside a ````
		// block does not close it - see lib/fence.js.
		if (fence !== null) {
			covered.add(i);
