...
const fs = require('fs');
const { fencedLineSet } = require('./fence');
const { normalize: normalizeFrontmatter } = require('./frontmatter');
...
function markedLines(lines) {
	...
	// Which lines are fenced is decided ONCE, with look-ahead, in lib/fence.js - see there.
	const fenced = fencedLineSet(lines);

	// A LEADING `---` BLOCK IS STRUCTURED DATA, NOT PROSE.
	//
	// `format`'s guarantee is "never changes a word", and it honoured that guarantee while destroying
	// every slice document in this repo - because it only JOINED LINES. In structured text a newline
	// is semantic, so one-key-per-line frontmatter collapsed into wrapped prose and the document
	// stopped being an object. `check` then reported it formatted correctly while `index` could not
	// parse it, and the failure mode depended on length: a long block wrapped and threw, a short one
	// - the five-line stub this model asks for - parsed SILENTLY into a garbage `id` that swallowed
	// every other field. See `DOCS-052`.
	//
	// Fenced code was already protected for exactly this reason, which is the argument that this is
	// the same class rather than a special case: regions where a newline carries meaning are copied
	// verbatim. The closing rule matches `lib/frontmatter.js` - a COMPLETE line, not a prefix - and an
	// unterminated block protects nothing, consistent with the marker rule below.
	if (lines.length && lines[0].trimEnd() === '---') {
		for (let i = 1; i < lines.length; i++) {
			if (lines[i].trimEnd() === '---') {
				for (let j = 0; j <= i; j++) { covered.add(j); }
				break;
			}
		}
	}
	...
	lines.forEach((line, i) => {
		...
		//
		// FENCED CODE IS VERBATIM TOO, AND IT HAS TO BE IN THIS SET, NOT JUST SKIPPED BY THE WRAPPER.
		//
		// The wrapper already left fences alone, so it looked protected. The DEFINITION paths did not:
		// they harvest every `[label]: url` line into the foot block and strip it from where it stood,
		// and they consult only this set. An example written inside a fence - which this toolkit's
		// own documentation is full of - was lifted out, had its trailing annotation dropped, and was
		// re-emitted at the foot as a LIVE link, leaving an empty fence behind. `DOCS-051` lost two
		// illustrations that way and gained three dead links, one of them to a literal `.../...` URL.
		// Same lesson as frontmatter in `DOCS-052`: protecting the wrapper is not protecting the
		// document. Every path that rewrites a line reads this one set.
		if (fenced.has(i)) {
			covered.add(i);
			return;
		}
...
function splitBlocks(lines, protectedLines = new Set()) {
	...
	for (const [i, line] of lines.entries()) {
		...
		// A fenced line is verbatim. Consecutive ones join a single block, as a fence did before.
		if (fenced.has(i)) {
			closeProse();
			const last = blocks[blocks.length - 1];
			if (last && last.kind === 'verbatim' && last.fenceRun === i - 1) {
				last.lines.push(line);
				last.fenceRun = i;
			} else {
				blocks.push({ kind: 'verbatim', lines: [line], fenceRun: i });
			}
			continue;
		}
	...
	if (buffer.length) { blocks.push({ kind: 'prose', lines: buffer }); }
	return blocks;
}
...
function format(input, options = {}) {
	...
	// Frontmatter values strict YAML rejects are re-quoted, and nothing else in the block is touched
	// (DOCS-066). GitHub renders the block as YAML; this is the repair for documents already written.
	const source = frontmatterNormalized(input);
	...
	// Move long URLs out of the prose, skipping fenced code - which is in `protectedLines`, because
	// markedLines takes it from lib/fence.js. There is no second fence tracker here to disagree with it.
	const converted = [];
	for (let i = 0; i < lines.length; i++) {
		const line = lines[i];
		const s = line.trimStart();

		// PROTECTING THE WRAPPER IS NOT PROTECTING THE DOCUMENT.
		//
		// `format` has more than one path that rewrites a line, and marking frontmatter unwrappable
		// left this one untouched: it harvested `doc: '[DOCS-001](slices/DOCS-001_....md)'` into
		// `doc: '[DOCS-001][docs-001]'` plus a definition appended to the BODY. The field still read fine, and
		// the definition - now outside the frontmatter - was resolved against the document's own
		// directory, so `links` reported 27 dead targets pointing at `slices/slices/...`.
		//
		// A marked region is not merely unwrappable, it is VERBATIM. Same rule as the fenced-code
		// guard on the next line, applied to the same set the wrapper already respects.
		if (protectedLines.has(i)) { converted.push(line); continue; }
 ...
 */
function frontmatterNormalized(source) {
	const lines = source.split('\n');
	if (!lines.length || lines[0].trimEnd() !== '---') { return source; }
	const close = lines.findIndex((l, i) => i > 0 && l.trimEnd() === '---');
	if (close === -1) { return source; }
	const block = lines.slice(1, close).join('\n');
	const fixed = normalizeFrontmatter(block);
	return fixed === block ? source : [lines[0], fixed, ...lines.slice(close)].join('\n');
}
