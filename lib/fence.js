...
/**
 * The line with any BLOCK-QUOTE prefix removed: `> `, `> > `, `>`.
 *
 * A fence inside a block quote starts with `>`, which is not whitespace, so a grammar anchored on
 * leading whitespace never saw it. The lazy regex this file replaced matched ``` anywhere on a line and
 * so happened to handle it - which made requiring the delimiter at the start of the line a REGRESSION
 * for quoted examples: links began reporting them as live. Codex, fifth pass on PR #5. Measured in the
 * estate: ewc3labs-hq has 8 block-quoted fence lines, excel-power-query-editor 2.
 *
 * One optional space after each `>` belongs to the quote marker; anything beyond it is the content's
 * own indentation, which is what the four-space rule below must still see.
 */
function unquote(line) {
	return line.replace(/^[ \t]*(?:>[ ]?[ \t]*?)*(?=[^>]|$)/, (prefix) => {
		const quoted = /^[ \t]*>/.test(prefix);
		if (!quoted) { return prefix; }
		// Keep whatever indentation follows the LAST marker's single optional space.
		const afterLast = prefix.slice(prefix.lastIndexOf('>') + 1);
		return afterLast.startsWith(' ') ? afterLast.slice(1) : afterLast;
	});
}

/** The opening run (e.g. "````") if this line opens a fence, else null. */
function fenceOpener(line) {
	const m = /^\s*(`{3,}|~{3,})(.*)$/.exec(unquote(line));
	if (!m) { return null; }
...
function closesFence(line, opener) {
	const m = /^\s*(`{3,}|~{3,})\s*$/.exec(unquote(line));
	return Boolean(m && m[1][0] === opener[0] && m[1].length >= opener.length);
}

/** Leading whitespace width of the line's CONTENT (after any quote prefix), a tab counted as four. */
function indentOf(line) {
	return (/^[ \t]*/.exec(unquote(line))[0]).replace(/\t/g, '    ').length;
}
 ...
 * and CLOSED, a fence. This needs look-ahead, which is why every tool reads this precomputed set
 * instead of keeping its own line-by-line state.
 *
 * INDENTED 4+ AND UNCLOSED IS GENUINELY AMBIGUOUS, AND THE SAFE ANSWER DEPENDS ON WHO IS ASKING.
 *
 * It is either indented code (`    ~~~` in an example - Codex's third pass on PR #5) or an unclosed fence
 * under a list item (Codex's fourth pass). Telling them apart needs a list-container parser, and a
 * hand-rolled one would be one more subset of CommonMark to get subtly wrong. The two readers of this
 * set have OPPOSITE safe defaults for exactly that uncertainty:
 *
 *   ambiguous: 'fenced'  format, values - protecting is safe. Verbatim cannot corrupt a document;
 *                        guessing "prose" joins code into paragraphs and substitutes example markers.
 *   ambiguous: 'open'    links - protecting is UNSAFE. It silently erases every link after the opener
 *                        from checking; guessing "not fenced" keeps checking, and its worst case is a
 *                        loud false positive inside a document that is already malformed.
 *
 * So this file decides WHAT is ambiguous, once, and each consumer declares its safe direction. Still
 * one grammar - the rule for an ambiguous line is not re-derived anywhere.
 */
function fencedLineSet(lines, { ambiguous = 'fenced' } = {}) {
	...
	while (i < lines.length) {
		...
		if (close === -1 && indentOf(lines[i]) >= 4 && ambiguous === 'open') { i++; continue; }
 ...
 *
 * For `links`, so an ambiguous opener is left OPEN: blanking it would silently drop every link after
 * it from checking. See fencedLineSet.
 */
function blankFences(text) {
	const lines = text.split('\n');
	const inside = fencedLineSet(lines, { ambiguous: 'open' });
